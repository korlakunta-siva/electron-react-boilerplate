...
export default class DataGrid extends React.Component {
 ...
 handleErrors = (response) => {
  ...
  handleStatement = () => {
    ...
    let row = this.getRows().filter( (elem,indx)=> indx == this.state.selectedIndexes)[0];
    console.log(row);
    fetch("https://192.168.1.15:86/statement?patid=" + row.PatientId.toString())
  ...
  handleTimerToggle = (encId, actiontype) => {
    ...
    "https://192.168.1.15:8044/exsql?dbserver=ecwSQL&sqltype=customSQL&sqltext=set%20rowcount%200%20 exec apex.pn.apexsp_saveEnvAction ";
