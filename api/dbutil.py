...
def main():
  ...
  parser.add_argument("-db", help="dbserver", default=None)
  parser.add_argument("-sql", help="sqltext", default=None)
  ...
  if command.strip() == 'runsql' :
    urllib3.disable_warnings()
    http = urllib3.HTTPSConnectionPool('192.168.21.199', port=8044, cert_reqs='CERT_NONE', assert_hostname=False)
...
# and imgserl_imgstr_id = imgstr_id and imgstr_imgsys_id = 2 and  imgserl_status = 'A') '''.format(patid)
    ...
    pat_exams_sql = sqltext

    rows = []
    try :
        urlstring="https://192.168.21.199:8044/exsql?dbserver=ecwSQL&sqltype=patientsToPurge&param1=5"
        urlstring="https://192.168.21.199:8044/exsql?dbserver=ecwSQL&sqltype=customSQL&sqltext=" + pat_exams_sql
