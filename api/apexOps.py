...
def main():
  ...
  if command.strip() == 'hoststatus' :  

    #   #command = ('C:\Windows\system32\quser.exe /server:' + hostname).split()
    #   command = "dir".split()
    #   for line in run_command(command):
    #      print(line)

        proc = sp.Popen(r'C:\\Windows\\system32\\quser.exe /server:apex-dt-23', stdout=sp.PIPE, stderr=sp.STDOUT)
...
#
  ...
  cmd_to_run = ''
  if command.strip() == 'quser' and hostname != None :
      cmd_to_run: 'c:\windows\squser /SERVER: ' +  str(hostname)
      processes = []
      for line in runProcess(cmd_to_run):
          print(line)
          line_args = str(line)[2:-1].split(',')
