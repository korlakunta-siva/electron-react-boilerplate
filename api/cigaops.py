...
import urllib3
import shlex
from iocmutil import getIOCMReprocessSQL, getCigConfig
import subprocess
import time


def sudo_run_commands_remote(command, server_address, server_username,
                             server_pass, server_key_file):
...
def unix_ps(request):
    ...
    for campus in ['R', 'J', 'S']:
        try:
            ...
            column_names = [col for col in head_rowline.split(',')
            ...
            html_table = html_table + "<h2 align='center'>CIG Processes in Campus: " + campus + " at: " + datetime.now(
            ).strftime("%m-%d-%Y %H:%M") + " </h2>"
            #df = pd.read_sql(dbspace_sql,cnxn)
            pd.set_option('display.max_columns', None)
            pd.set_option('display.max_colwidth', -1)
            df = pd.DataFrame(
                ps_output
            )  #.sort_values(['server', 'database', 'table'], ascending=[1, 1, 1])
            df_receivers = pd.DataFrame(ps_receivers)
            df_processors = pd.DataFrame(ps_processors)

            html_table = html_table + "<h3 align='center'>Receivers : " + str(
                len(ps_receivers)) + "</h3>" + df_receivers.to_html()
            html_table = html_table + "<h3 align='center'>Processors : " + str(
                len(ps_processors)) + "</h3>" + df_processors.to_html()
            #print(html_table)
...
$.ajaxSetup({
    ...
    output1 = render_dynamic_djangoview(
        request, template_string, 'dynamic_view', {
            'title': 'Dashboard',
...
def live_ciga_scan_df(campus):
    ...
    # stdin, stdout, stderr = ssh.exec_command(
    #     "sudo su - ciguser")
    stdin, stdout, stderr = ssh.exec_command("sudo -u ciguser df -g")
    ...
    currentdate = (datetime.datetime.now() +
                   datetime.timedelta(days=1)).strftime("%m-%d-%Y 10:10:10")
...
def main():
    ...
    if command.strip() == 'iocmreprocess':
        sqlText = getIOCMReprocessSQL()
        CigConfig = getCigConfig()
        receivers = CigConfig['Receivers']
        #print("Started test DB", receivers)
        ...
        try:
            ...
            urlstring = "https://iasq1mr2:8081/exsql?dbserver=iimsProd&sqltype=customSQL&sqltext=" + sqlText
            r = http.request('GET', urlstring)
            r.release_conn()
            data_frame = json.loads(r.data.decode('utf-8'))
            rows = json.loads(data_frame['frame0'])['rows']
            #print("iocm rows:", rows)
            #return rows
        except Exception as ex:
            print('Error Calling URL for iocmrows. Error:  %s ' % (ex))
            sys.exit(0)
        finally:
            pass

        for series in rows:
            print("Series", series['series_uid'])
            storePath = series['qreads_store_path']
            seriesFile = series['qreads_series_file_name']
            seriesFilePath = storePath + '\\' + seriesFile
            if os.path.exists(seriesFilePath):
                print("KO Folder exists")
            else:
                print("KO Folder DOES NOT EXIST")
                continue

            print(seriesFilePath)
            midiaDestAET = series['dest_desc']
            cigaRecentDestAET = series['ciga_dest_aet']

            #print(midiaDestAET, cigaRecentDestAET)

            destReceiverAET = ""
            if (midiaDestAET != '' and midiaDestAET != None):
                destReceiverAET = midiaDestAET[:12] + 'STG'
            elif (cigaRecentDestAET != '' and cigaRecentDestAET != None):
                destReceiverAET = cigaRecentDestAET[:12] + 'STG'
            else:
                pass

            if (destReceiverAET == ""):
                print("UNABLE TO FIND DESTINATION RECEIVER AET")
                continue
                pass

            print(destReceiverAET)
            destReceiver = None
            for rcvr in receivers:
                if rcvr['recvaet'] == destReceiverAET and rcvr[
                        'queue'] == 'preprod':
                    destReceiver = rcvr
                    break

            if (destReceiver == None):
                print("UNABLE TO FIND DESTINATION RECEIVER")
                continue
                pass

            #print(destReceiver)
            senderAET = destReceiver['sendaet1']
            receiverArg = destReceiver['recvaet'] + '@' + destReceiver[
                'ipaddr'] + ':' + destReceiver['port']

            programPath = "C:\\Programs\\dcm4che\\bin\\storescu"

            print(receiverArg, programPath, senderAET)

            cmdOut = subprocess.check_output(
                'cmd.exe /C ' + programPath + " -b " + senderAET + " -c " +
                receiverArg + " " + seriesFilePath,
                shell=False)
    ...
    if command.strip() == 'testdb':
        ...
        try:
            ...
            print('Error Calling URL for patientToPurgetid. Error:  %s ' %
                  (ex))
        finally:
            pass
    ...
    if command.strip() == 'testes':
        ...
        json_query_data = json.dumps({
            "query":
            "SELECT patient_cmrn, count(*) FROM \"imsu-dashboardusers___cis_storage*\" group by patient_cmrn limit 1000 "
        })
        json_query_data = json.dumps({
            "query":
            "SELECT image_store_name, sum(image_count) FROM \"imsu-dashboardusers___cis_storage*\" group by image_store_name limit 100 "
        })
        json_query_data = json.dumps({
            "query":
            "SELECT * FROM \"imsu-dashboardusers___cis_storage*\" order by patient_cmrn limit 100 "
        })
        ...
        json_query_data = json.dumps({
            "query":
            "SELECT   'iims_pending' as \"indexname\", min(event_at) as begin_at, max(event_at) as end_at,  (CAST(now() AS long) - cast(max(event_at) as long) ) / 1000 AS \"diffInSec\" ,  (-3600 + (CAST(now() AS long) - cast(max(event_at) as long) ) / 1000) / 86400 AS \"diffIndays\"  FROM \"imsu-dashboardusers___iims_pending*\" "
        })
        ...
        query_result = map(
            lambda row: dict(zip([col['name']
                                  for col in res['columns']], row)),
            res['rows'])
        ...
        json_query_data = json.dumps({
            "query":
            "SELECT   'vip_queues' as \"indexname\", min(event_at) as begin_at, max(event_at) as end_at,  (CAST(now() AS long) - cast(max(event_at) as long) ) / 1000 AS \"diffInSec\" , (-3600 + (CAST(now() AS long) - cast(max(event_at) as long) ) / 1000) / 86400 AS \"diffIndays\" FROM \"imsu-dashboardusers___iims_vip_queues*\" "
        })
        res = es.sql.query(body=json_query_data, format='json')
        query_result = map(
            lambda row: dict(zip([col['name']
        ...
        json_query_data = json.dumps({
            "query":
            "SELECT   'ciga_jobs' as \"indexname\", min(event_at) as begin_at, max(event_at) as end_at,  (CAST(now() AS long) - cast(max(event_at) as long) ) / 1000 AS \"diffInSec\" , (-3600 + (CAST(now() AS long) - cast(max(event_at) as long) ) / 1000) / 86400 AS \"diffIndays\" FROM \"imsu-dashboardusers___ciga_jobs*\" "
        })
        res = es.sql.query(body=json_query_data, format='json')
        query_result = map(
            lambda row: dict(zip([col['name']
        ...
        json_query_data = json.dumps({
            "query":
            "SELECT   'cis_storage' as \"indexname\", min(event_at) as begin_at, max(event_at) as end_at,  (CAST(now() AS long) - cast(max(event_at) as long) ) / 1000 AS \"diffInSec\" , (-3600 + (CAST(now() AS long) - cast(max(event_at) as long) ) / 1000) / 86400 AS \"diffIndays\" FROM \"imsu-dashboardusers___cis_storage*\" "
        })
        res = es.sql.query(body=json_query_data, format='json')
        query_result = map(
            lambda row: dict(zip([col['name']
        ...
        json_query_data = json.dumps({
            "query":
            "SELECT   'qr_performance' as \"indexname\", min(event_at) as begin_at, max(event_at) as end_at,  (CAST(now() AS long) - cast(max(event_at) as long) ) / 1000 AS \"diffInSec\" , (-3600 + (CAST(now() AS long) - cast(max(event_at) as long) ) / 1000) / 86400 AS \"diffIndays\" FROM \"imsu-dashboardusers___qreads_perfmertrics_20*\" "
        })
        res = es.sql.query(body=json_query_data, format='json')
        query_result = map(
            lambda row: dict(zip([col['name']
    ...
    if command.strip() == 'testps':
        ...
        try:
            ...
            html_table = html_table + "<h2 align='center'>CIG Processes in Campus: " + campus + " at: " + datetime.datetime.now(
            ).strftime("%m-%d-%Y %H:%M") + " </h2>"
            #df = pd.read_sql(dbspace_sql,cnxn)
            pd.set_option('display.max_columns', None)
    ...
    if command.strip() == 'cigcmd':
        ...
        cmdtorun = arguments
        try:
            cmd_out = run_ciga_unix_cmd(hostname=hostname,
                                        unixcommand=cmdtorun)
            print(cmd_out)
        except Exception as ex:
            print(ex)
            pass
